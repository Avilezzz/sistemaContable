...
def generar_pdf_libro_diario(db: Session, nombre_archivo="libro_diario.pdf"):
    ...
    if empresa:
        # Crear encabezado profesional con período de fechas
        elements.extend(
    ...
    for asiento in asientos:
        ...
        desc_asiento = Paragraph(
            f"<b>Asiento #{asiento.id}:</b> {asiento.descripcion}", 
            styles['Normal']
        )
    ...
    for asiento in asientos:
        ...
        # 2) PRIMERO AGREGAMOS TODAS LAS DEL DEBE (ordenadas)
    ...
    if abs(total_debe_general - total_haber_general) < 0.01:
        validacion = Paragraph(
            "<b>✓ Libro Diario Cuadrado (Debe = Haber)</b>", 
            ParagraphStyle('Valid', parent=styles['Normal'], 
                          textColor=colors.green, alignment=1, fontSize=9)
        )
    else:
        validacion = Paragraph(
            f"<b>⚠ ADVERTENCIA: Descuadre de ${abs(total_debe_general - total_haber_general):,.2f}</b>", 
